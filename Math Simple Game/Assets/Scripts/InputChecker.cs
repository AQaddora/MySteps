...
public class InputChecker : MonoBehaviour
{
	...
	{
		string inputString = input.text.Trim();
		Debug.Log(CheckNumber(inputString));
		if ((CheckNumber(inputString)).Equals(correctInput))
		{
			correct = true;
			...
			if(GetComponentInParent<QuestionSetManager>() != null) GetComponentInParent<QuestionSetManager>().CheckIfQuestionSetFinished();
			if (GetComponentInParent<lengthLevel2Manager>() != null) GetComponentInParent<lengthLevel2Manager>().NextStage();
		}
		else
		{
